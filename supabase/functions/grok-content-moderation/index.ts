...
serve(async (req) => {
    ...
    try {
        ...
        // 先用本地正则快速拦截（无论 AI 是否可用都执行）
        const localCheck = checkLocalFallback(textToAudit);
        ...
        const systemPrompt =
`你是一个加拿大华人社区平台（渥帮JWD）的高级内容安全与风控审核员。
请严格审查用户提交的社区发帖、服务介绍或评论文本。
重点识别并拦截以下违规分类：
1. 【各类黄色与色情信息】：
   - 色情描写、露骨性暗示、涉黄游戏（如"色情游戏"、"成人游戏"）；
   - 成人交友平台引流（如"成人交友"、"一夜情"、"约炮"、"援交"）；
   - 同城招嫖、上门服务暗语（如"妹子上门"、"小姐上门"、"全套服务"）；
   - 裸聊、黄色视频/图片引流（如"看片"、"福利视频"、"私信领福利"）；
   - 成人用品/情趣用品违规推销、色情网站/APP推广；
...
重要提示：即使内容措辞温和或隐晦（如仅含"成人交友"、"色情游戏"等字眼），只要指向上述违规场景，必须设置 flagged 为 true。
...
{"flagged": true, "reason": "包含涉嫌色情、招嫖或违规信息"} 或 {"flagged": false}`;
        ...
        // Final fallback: re-run local regex (already passed, so return safe)
        return new Response(JSON.stringify({ flagged: false }), {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
