...
const Login = () => {
    ...
    // wall. Hide both there; phone/email login still works fine (no
    // off-domain redirect involved).
    const showGoogleApple = !inMiniProgram;
    const hasOAuthOptions = showWeChatLogin || showGoogleApple;
    ...
    const t = {
        ...
        thirdPartyLogin: language === 'zh' ? '快捷登录' : 'Quick sign in',
    ...
    const handleWeChatLoginClick = async () => {
        ...
        try {
            ...
            window.wx.miniProgram.navigateTo({
                url: `/pages/quick-login/quick-login?returnPath=${encodeURIComponent(returnPath)}`,
                success: () => console.log('navigateTo quick-login succeeded'),
                fail: (err) => {
                    console.error('navigateTo quick-login failed', err);
                    toast.error(t.errSocialFailed('Login'));
                },
            });
        } catch (err) {
                ...
                <div className="p-8 lg:p-12">
                    <div className="max-w-md mx-auto h-full flex flex-col justify-center">
                        ...
                        <div className="space-y-6">
                            {/* Quick sign-in (WeChat/Google/Apple) — leads ahead of the
                                phone/email form since it's the fastest path for most
                                returning users. Hidden entirely if none are available
                                (e.g. inside the Mini Program's web-view). */}
                            {hasOAuthOptions && (
                                <div className="space-y-4">
                                        ...
                                        {showWeChatLogin && (
                                            inMiniProgram ? (
                                                <button
                                                    ...
                                                    disabled={loading}
                                                    aria-label={language === 'zh' ? '手机号快捷登录' : 'Quick Mobile Sign-In'}
                                                    title={language === 'zh' ? '手机号快捷登录' : 'Quick Mobile Sign-In'}
                                                    className="w-full py-4 px-4 border-2 border-primary/20 bg-primary/5 hover:bg-primary/10 text-primary font-bold rounded-2xl flex items-center justify-center gap-2 transition-all disabled:opacity-50"
                                                >
                                                    <Smartphone className="w-5 h-5" />
                                                    <span>{language === 'zh' ? '手机号快捷登录' : 'Quick Mobile Sign-In'}</span>
                                                </button>
                                            ) : (
                                                <button
                                                    onClick={handleWeChatLoginClick}
                                                    disabled={loading}
                                                    aria-label={language === 'zh' ? '快捷登录' : 'Quick Sign-In'}
                                                    title={language === 'zh' ? '快捷登录' : 'Quick Sign-In'}
                                                    className="flex-1 max-w-[88px] flex items-center justify-center py-4 border-2 border-slate-100 rounded-2xl hover:bg-slate-50 hover:border-slate-200 transition-all disabled:opacity-50 text-slate-700 hover:text-primary"
                                                >
                                        ...
                                        {showGoogleApple && (
                                            <>
                                                <button
                                                    ...
                                                    aria-label="Google"
                                                    title="Google"
                                                    className="flex-1 max-w-[88px] flex items-center justify-center py-4 border-2 border-slate-100 rounded-2xl hover:bg-slate-50 hover:border-slate-200 transition-all disabled:opacity-50"
                                                >
                                    ...
                                    <div className="relative pt-2">
                                        ...
                                        <div className="relative flex justify-center">
                                            <span className="bg-white px-3 text-xs font-bold text-slate-400 uppercase tracking-widest">{t.orDivider}</span>
                                        </div>
                                    </div>
                                </div>
                            )}
